...
impl Default for CtrlInfo {
    fn default() -> CtrlInfo {
        ...
        if !isK8s {
            ...
            let lab1ip;
            #[cfg(offload = "yes")]{
                lab1ip = u32::from(Ipv4Addr::from_str("192.168.2.27").unwrap()).to_be();
            }
            #[cfg(not(offload = "yes"))]{
                lab1ip = u32::from(Ipv4Addr::from_str("192.168.2.7").unwrap()).to_be();
            }
            let node1 = Node {
            ...
            let lab2ip;
            #[cfg(offload = "yes")]{
                lab2ip = u32::from(Ipv4Addr::from_str("192.168.2.25").unwrap()).to_be();
            }
            #[cfg(not(offload = "yes"))]{
                lab2ip = u32::from(Ipv4Addr::from_str("192.168.2.5").unwrap()).to_be();
            }
            let node2 = Node {
        ...
        CtrlInfo {
            ...
            podIpInfo: Mutex::new(HashMap::from_iter([(
                ...
                (u32::from(Ipv4Addr::from_str("192.168.2.27").unwrap()).to_be()),
            )])),
